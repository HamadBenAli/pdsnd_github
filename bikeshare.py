...
def get_filters():
    ...
    while True:
        city = input('Select city by name to analyze\n1.Chicago\n2.New York City\n3.Washington\n').lower()
    ...
    while True:
         month = input('Select month by name from january until june to filter , or all to apply no month filter\n').lower()
    ...
    while True:
        day = input('Select one day by name from the week to filter, or "all" to apply no day filter\n').lower()
        if day not in days:
...
def station_stats(df):
    ...
    most_used_start_station = df['Start Station'].value_counts().head(1).idxmax()
...
def main():
    while True:
        ...
        row_of_data = df.head()
        print(row_of_data)
        num_of_rows = int(input("Want to see more rows? Please enter number of rows you want to see\n"))
